...
import { ingredientSummary, markColor, priceStr, SEED_DISHES, type DishCategory } from "@/lib/menu-seed";

export function BloomCafeMenuContent() {
  const [tab, setTab] = useState<DishCategory>("veg");
  const filtered = SEED_DISHES.filter((d) => d.cat === tab);
  const featured = filtered[0] ?? null;
  const rest = filtered.slice(1);

  return (
    <div className="mx-auto flex min-h-dvh max-w-md flex-col bg-background pb-10 font-sans text-[oklch(0.28_0.02_60)]">
      <PhoneHero height={190} />
